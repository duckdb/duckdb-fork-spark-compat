...
static unique_ptr<QueryNode> BuildDescribeSelect(const DescribeTarget &describe_target, const string &function_name) {
	...
	if (describe_target.is_table_name) {
		...
		auto &table_qname = base_table.GetQualifiedName();
		if (!IsInvalidCatalog(table_qname.Catalog())) {
			target_name += table_qname.Catalog().GetIdentifierName() + ".";
		}
		if (!IsInvalidSchema(table_qname.Schema())) {
			target_name += table_qname.Schema().GetIdentifierName() + ".";
		}
		target_name += table_qname.Name().GetIdentifierName();
	} else {
